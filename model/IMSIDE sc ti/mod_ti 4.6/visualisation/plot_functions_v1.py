...
def plot_next(self, sss, version):
    ...
    pt1 = 1j * tid_geg['omega'] * tid_geg['eta'][0,:,0] * self.H**2/self.Kv_st * dsbdx * (u_bar * p3_z + alf * dsbdx * p4_z)
    pt2 = - self.g/(1j*tid_geg['omega']) * (tid_geg['detadx2'][0,:,0] + tid_geg['detadx'][0,:,0]/self.bex) * self.H**2/self.Kv_st * dsbdx * p34w
    pd_guess = pt1 + pt2
