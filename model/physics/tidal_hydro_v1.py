...
from model.physics.tide_funcs5 import sti_coefs
...
def tide_calc(self):
    ...
    for key in self.ch_keys:  
        ...
        self.ch_pars[key]['B'] = (np.cosh(self.ch_pars[key]['deA']) +  self.ch_pars[key]['rr_ti'] * self.ch_pars[key]['deA'] * np.sinh(self.ch_pars[key]['deA']))**-1
        ...
        Av_ti_here = np.repeat(self.ch_pars[key]['Av_ti'] , self.ch_pars[key]['nxn'])
        rr_ti_here = np.repeat(self.ch_pars[key]['rr_ti'] , self.ch_pars[key]['nxn'])[np.newaxis,:,np.newaxis]
        
        self.ch_pars[key]['Kv_ti_dom'] = np.repeat(self.ch_pars[key]['Kv_ti'] , self.ch_pars[key]['nxn'])[np.newaxis,:,np.newaxis]
        self.ch_pars[key]['deA_dom']   = ((1+1j)*self.ch_pars[key]['H']/np.sqrt(2*Av_ti_here/self.omega))[np.newaxis,:,np.newaxis]
        self.ch_pars[key]['B_dom'  ]   = ((np.cosh(self.ch_pars[key]['deA_dom']) + rr_ti_here * self.ch_pars[key]['deA_dom'] * np.sinh(self.ch_pars[key]['deA_dom']))**-1)
    ...
    for key in self.ch_keys:
        ...
        # =============================================================================
        c1 = - self.ch_pars[key]['Kv_ti_dom']/(1j*self.omega)
        ...
        for dom in range(self.ch_pars[key]['n_seg']):
            ...
            dstci   = np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * dstc
            dstcpi  = np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * dstcp
            dstc_zi = np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * dstc_z

            dstc_xL.append(dstc)
            dstc_xLi.append(dstci)
            dstc_x_xLi.append(dstci * -1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
            dstcp_x_xLi.append(dstcpi * -1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
            ...
            dstc_x_x0i.append(dstci * 1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
            dstcp_x_x0i.append(dstcpi * 1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
