...
def forc_RMD20():
    ...
    Qriv = np.array([600 + np.zeros(T), 50 + np.zeros(T)]) #mind the sign! this is the discharge at r1,r2,...
    Qweir= np.array([0 + np.zeros(T), 125 + np.zeros(T), 0 + np.zeros(T), 0 + np.zeros(T)])
    Qhar = np.array([0 + np.zeros(T)])
    n_sea = np.array([0 + np.zeros(T)]) #this is the water level at s1,s2,...
    soc = np.array([35 + np.zeros(T)])  # this is salinity at s1,s2, ...
    sri = np.array([0.15 + np.zeros(T), 0.15 + np.zeros(T)])  # this is the salinity of the river water at r1,r2, ...
    swe = np.array([0.15 + np.zeros(T), 0.15 + np.zeros(T), 0.15 + np.zeros(T), 0.15 + np.zeros(T)])
    tid_per = 44700
    a_tide = np.array([0.83])  # this is the amplitude of the tide at s1,s2, ...
    p_tide = np.array([70 * np.pi / 180])
...
def forc_RMD_fromcsv_old(loc, fil, ssea=33):
    ...
    # forcing conditions
    Qriv = np.array([dat_raw['Waal'], dat_raw['Maas']])  # mind the sign! this is the discharge at r1,r2,...
    Qweir = np.array([dat_raw['Hollandsche IJssel'], dat_raw['Lek']])  # ,0,0]
    Qhar = np.array([dat_raw['Haringvliet']])
    n_sea = np.array([0 + np.zeros(T)])  # this is the water level at s1,s2,...
    soc = np.array([35 + np.zeros(T)])  # this is salinity at s1,s2, ...
    sri = np.array([0.15 + np.zeros(T), 0.15 + np.zeros(T)])  # this is the salinity of the river water at r1,r2, ...
    swe = np.array([0.15 + np.zeros(T), 0.15 + np.zeros(T)])
