...
def calc_hourly_salinity(self, key, xi, zi, ret):
    # =============================================================================
    ...
    if ret == 'hourly':
        ...
        s_st = self.ch_outp[key]['s_st'][:,xi,zi]
        return self.Tvec, s_st, None
        
    else: print('No known time format')

    

def plot_salt_pointRM(self,locE,locN,depth, makeplot = True, ret = 'hourly'):
    # =============================================================================
    ...
    tvec, sst, sti = calc_hourly_salinity(self, ind_ch, ind_co, depth_ind, ret)
    
    if makeplot == True: 
        #plot
        plt.plot(tvec,sst+sti,'red',linewidth=2,label='mod',alpha = 0.5)
        plt.plot(tvec,sst,'red',linewidth=2,label='mod')
        plt.grid(),plt.legend()
        plt.ylabel('Salinity [g/kg]')
        #plt.xlim(0,30)
        plt.show()
        
    if ret == 'hourly': return tvec, sst+sti
    elif ret == 'daily': return tvec, sst
    else: print('No known time format')

# plot_salt_pointRM(delta,4.2,51.95,1)
# plot_salt_pointRM(delta,4.5,51.85,1)
# plot_salt_pointRM(delta,4.5,51.92,1)
