...
import numpy as np
import time as time
from model.physics.tide_funcs5 import sti
...
def tidal_salinity(self,key,zout):
    ...
    di_here = self.ch_pars[key]['di']
    nph = self.nn*np.pi/self.ch_pars[key]['H'][:,np.newaxis]
    ...
    for dom in range(self.ch_pars[key]['n_seg']): 
        ...
        c3[:,di_here[dom]:di_here[dom+1]] = - nph[:,di_here[dom]:di_here[dom+1]] * sn[:,di_here[dom]:di_here[dom+1]] * self.ch_pars[key]['eta'][:,di_here[dom]:di_here[dom+1]]
        c4[:,di_here[dom]:di_here[dom+1]] = - nph[:,di_here[dom]:di_here[dom+1]] * sn[:,di_here[dom]:di_here[dom+1]] * self.go2 \
    ...
    for dom in range(self.ch_pars[key]['n_seg']): 
        ...
        dc3dx[:,di_here[dom]:di_here[dom+1]] = - nph[:,di_here[dom]:di_here[dom+1]] * (self.ch_pars[key]['eta'][:,di_here[dom]:di_here[dom+1]]*dsndx[:,di_here[dom]:di_here[dom+1]] + sn[:,di_here[dom]:di_here[dom+1]]*self.ch_pars[key]['detadx'][:,di_here[dom]:di_here[dom+1]])
        dc4dx[:,di_here[dom]:di_here[dom+1]] = - nph[:,di_here[dom]:di_here[dom+1]] * self.go2 * (dsndx[:,di_here[dom]:di_here[dom+1]] * (self.ch_pars[key]['detadx2'][:,di_here[dom]:di_here[dom+1]] + self.ch_pars[key]['detadx'][:,di_here[dom]:di_here[dom+1]]/self.ch_pars[key]['bn'][dom]) \
    ...
    for dom in range(self.ch_pars[key]['n_seg']):
        ...
        stc_z = np.sum((zout[self.ch_inds[key]['di3'][dom]*self.M : (self.ch_inds[key]['di3'][dom]+1)*self.M] + 1j*zout[(self.ch_inds[key]['di3'][dom]+1)*self.M : (self.ch_inds[key]['di3'][dom]+2)*self.M])[:,np.newaxis] \
                       * -self.m0[:,np.newaxis]*np.pi/self.ch_gegs[key]['Hn'][dom] * np.sin(np.pi*self.m0[:,np.newaxis]*self.z_nd) , axis=0) * self.soc_sca
        #calculate x 
        x_all = np.arange(di_here[dom+1]-di_here[dom])*self.ch_gegs[key]['dx'][dom] 
        x_bnd = x_all[np.where(x_all<(-np.sqrt(self.ch_pars[key]['epsL'][dom])*np.log(self.tol)))[0]]
        if x_bnd[-1] >= self.ch_gegs[key]['L'][dom]: print('ERROR: boundary layer too large. Can be solved...')
        #salinity in inner domain
        stci = np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * stc
        stcpi= np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * stcp
        stc_zi= np.exp(-x_bnd[:,np.newaxis]/np.sqrt(self.ch_pars[key]['epsL'][dom])) * stc_z

        #save
        stc_xL.append(stc)
        stc_xLi.append(stci)
        stc_x_xLi.append(stci * -1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
        stcp_x_xLi.append(stcpi * -1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
        ...
        stc_x0.append(stc)
        stc_x0i.append(stci)
        stc_x_x0i.append(stci * 1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
        stcp_x_x0i.append(stcpi * 1/np.sqrt(self.ch_pars[key]['epsL'][dom]))
...
def func_jac_int(self, key):
    ...
    for dom in range(self.ch_pars[key]['n_seg']):
        ...
        nph         = self.nn*np.pi/self.ch_gegs[key]['Hn'][dom]
...
def func_jac_intz(self,key): 
    ...
    for dom in range(self.ch_pars[key]['n_seg']):     
        ...
        dx_here      = self.ch_gegs[key]['dx'][dom]
        nph          = self.nn*np.pi/self.ch_gegs[key]['Hn'][dom]
        zlist        = self.ch_pars[key]['zlist'][:,di_here[dom]:di_here[dom+1]]
        dsdc2_here   = self.ch_pars[key]['c2c'][:,di_here[dom]:di_here[dom+1]]
        dsdc3_here   = self.ch_pars[key]['c3c'][:,di_here[dom]:di_here[dom+1]]
        dsdc4_here   = self.ch_pars[key]['c4c'][:,di_here[dom]:di_here[dom+1]]
...
def func_st_after(key,sn,dsndx,dsbdx,dsbdx2,bb):       
    ...
    if self.ch_gegs[key]['loc x=-L'][0] == 'j': 
        ...
        cor_dom = np.exp(-px_here/np.sqrt(self.ch_pars[key]['epsL'][dom]))[:,np.newaxis] *  np.sum(bb[0][:,np.newaxis] * np.cos(np.pi*self.m0[:,np.newaxis]*sigma) , axis=0)
        st_cor += self.soc_sca * cor_dom

    if self.ch_gegs[key]['loc x=0'][0] == 'j': 
        px_here = self.ch_outp[key]['px']
        cor_dom = np.exp(px_here/np.sqrt(self.ch_pars[key]['epsL'][dom]))[:,np.newaxis] *  np.sum(bb[1][:,np.newaxis] * np.cos(np.pi*self.m0[:,np.newaxis]*sigma) , axis=0)
        st_cor += self.soc_sca * cor_dom
    '''
    return st_cor
...
def jac_tidal_fix(self, key):
    ...
    jac[inds['xnr_m'], inds['xnrm_mj']] += tid_raw[3][inds['xr'],inds['j1']]
    #center
    ...
    jac[inds['xnr_m'], inds['xnrp_mj']] += tid_raw[5][inds['xr'],inds['j1']]
    ...
    for dom in range(self.ch_pars[key]['n_seg']):   
        ...
        #depth-varying
        istart = self.ch_inds[key]['di3'][dom]
        istop  = self.ch_inds[key]['di3'][dom]+len_xL-1
        x0 = np.arange(istart , istop)
        if len_xL > 1 :     
            for i1 in range(len(x0)):
                for i2 in range(self.N):
                    for i3 in range(self.M):
                        jac[(x0[i1]+3)*self.M + i2 + 1, x0[0]*self.M + i3 ] += tid_cor[2][dom][i3,i2,i1+1]
                        jac[(x0[i1]+3)*self.M + i2 + 1, x0[0]*self.M + i3 + self.M ] += tid_cor[3][dom][i3,i2,i1+1]
        ...
        #depth-varying
        istart = self.ch_inds[key]['di3'][dom+1]-len_x0+1
        istop  = self.ch_inds[key]['di3'][dom+1]
        x0 = np.arange(istart , istop)
        #print(x0)
        if len_x0 > 1 :     
            for i1 in range(len(x0)):
                for i2 in range(self.N):
                    for i3 in range(self.M):
                        #print(tid_cor[6][dom].shape  , i3,i2,i1+1)
                        jac[(x0[i1]-3)*self.M + i2 + 1, (istop-2)*self.M + i3 ] += tid_cor[6][dom][i3,i2,i1]
                        jac[(x0[i1]-3)*self.M + i2 + 1, (istop-2)*self.M + i3 + self.M ] += tid_cor[7][dom][i3,i2,i1]
