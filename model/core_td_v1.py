...
class mod42_netw:
    ...
    def run_checks(self):
        ...
        for i in range(len(self.ends)):
            if self.ends[i][0] == 'r':
                ends_r.append(self.ends[i])
            elif self.ends[i][0] == 's':
                ends_s.append(self.ends[i])
            elif self.ends[i][0] == 'j':
                ends_j.append(self.ends[i])
            elif self.ends[i][0] == 'w':
                ends_w.append(self.ends[i])
            elif self.ends[i][0] == 'h':
        ...
        for i in range(len(self.ends)):
            if self.ends[i][0] == 'r':
                ...
                print('ERROR: unrecognised channel ending')
        ...
        for key in self.ch_keys:
            if self.ch_gegs[key]['loc x=-L'][0] == 's':
                print(
                    'SERIOUS WARNING: please do not use a sea boundary at x=-L. There is some error, I cannot find out where it is. To repair later.  ')
                print('Expect the code to crash')

        # check labelling of vertices
        ends_j = list(set(ends_j))  # added this and commented out line below, this removes check for 3* each junction.
        ...
        for i in range(self.n_j):  ends_j2.append(int(ends_j[i][1:]))
        #ends_j2 = np.sort(ends_j2).reshape((self.n_j, 3))[:, 0]  # every junction point has to appear three times
        if not np.array_equal(np.sort(ends_r2), np.arange(self.n_r) + 1): print(
            'ERROR: something wrong with the labelling of the river points')
        if not np.array_equal(np.sort(ends_s2), np.arange(self.n_s) + 1): print(
            'ERROR: something wrong with the labelling of the sea points')
        if not np.array_equal(np.sort(ends_w2), np.arange(self.n_w) + 1): print(
            'ERROR: something wrong with the labelling of the weir points')
        if not np.array_equal(np.sort(ends_h2), np.arange(self.n_h) + 1): print(
            'ERROR: something wrong with the labelling of the har points')
        if not np.array_equal(np.sort(ends_j2), np.arange(self.n_j) + 1): print(
            'ERROR: something wrong with the labelling of the junction points')
        if np.max(np.abs(self.Qriv), initial=0) > 50000: print(
            'WARNING: values of river discharge are unlikely high, please check')
        if np.max(np.abs(self.Qweir), initial=0) > 50000: print(
            'WARNING: values of weir discharge are unlikely high, please check')
        if np.max(np.abs(self.Qhar), initial=0) > 50000: print(
            'WARNING: values of har discharge are unlikely high, please check')
        if np.max(np.abs(self.n_sea), initial=0) > 1: print(
            'WARNING: values of sea surface height are unlikely high, please check')
        if np.max(self.soc, initial=0) > 40 or np.min(self.soc, initial=0) < 0: print(
            'WARNING: values of sea salinity are unlikely, please check')
        if np.max(self.sri, initial=0) > 40 or np.min(self.sri, initial=0) < 0: print(
            'WARNING: values of river salinity are unlikely, please check')
        if np.max(self.swe, initial=0) > 40 or np.min(self.swe, initial=0) < 0: print(
            'WARNING: values of weir salinity are unlikely, please check')
        if np.max(self.a_tide) > 5: print('WARNING: values of tidal surface amplitude are unlikely, please check')

        for key in self.ch_keys:
            if np.min(self.ch_pars[key]['H']) < 0 or np.max(self.ch_pars[key]['H']) > 50: print(
                'WARNING: values of depth are unlikely, please check')
            if np.min(self.ch_gegs[key]['b']) < 0: print('WARNING: width can not be negative')
            if np.min(self.ch_gegs[key]['dx']) < 0 or np.max(self.ch_gegs[key]['dx']) > 50000: print(
                'WARNING: values for spatial grid step are unlikely, please check')
            if np.min(self.ch_pars[key]['nxn']) < 4: print(
                "WARNING: in the channel " + key + ' is a channel which has less than 4 horizontal elements, which can give problems')
    ...
    from model.physics.discharge_distribution_v1 import Qdist_calc
    #calculation of tidal properties
    from model.physics.tidal_hydro_v1 import tide_calc
    #subtidal modules
    from model.physics.subtidal_v1 import subtidal_module , sol_subtidal, jac_subtidal_fix,  jac_subtidal_vary
    #solution
    from model.physics.solve_td_v1 import run_model
    #from solve_ti_v1_check import run_model

    from model.physics.indices_v1 import indices

    from model.physics.tidal_salt_v2 import tidal_salinity, sol_tidal, jac_tidal_fix, jac_tidal_vary

    from model.physics.inertiaterm_v1 import sol_inertia, sol_inertia_o, sol_inertia_n , jac_inertia

    from model.physics.boundaries_v1 import sol_bound, jac_bound_fix, jac_bound_vary

    from model.physics.junctions_v2 import prep_junc, sol_junc_tot, jac_junc_tot_fix, jac_junc_tot_vary

    from model.visualisation.calc_rawtofine_td_v1 import calc_output



    #visualisation
    from model.visualisation.visu_RM_td import plot_salt_pointRM , anim_RM_st
    from model.visualisation.calc_RM_td import calc_X2_td
    from model.visualisation.visu_gen_td import plot_s_gen_td
    ...
    from calc_rawtofine_v2 import calc_output
    # =============================================================================
    # Import the functions for the different visualisations
    # =============================================================================
    #visualisation of tidal properties
    from visu_tide_v1 import plot_tide , plot_salt_ti_point, plot_tide_timeseries
    ...
    from visu_RM import plot_new_compRM, plot_salt_compRM, anim_new_compRM ,  plot_tide_pointRM
    from plot_verts import plot_proc_vt
    # =============================================================================
    #  error calculation, for tuning
    # =============================================================================
    from calc_tide_v1 import calc_tide_pointRM, max_min_salts
    from calcs_afterwards import calc_X2
    '''
