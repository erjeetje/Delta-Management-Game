...
def calc_tide_pointRM(self):
    ...
    return score
    '''
    fig, model_output_ax = plt.subplots(2,1,figsize = (5,5))
    model_output_ax[0].scatter(punten[np.where(~np.isnan(aM2_mod))[0]],amp_M2[np.where(~np.isnan(aM2_mod))[0]],marker='D',label='obs',color='blue')
    model_output_ax[0].scatter(punten[np.where(~np.isnan(aM2_mod))[0]],aM2_mod[np.where(~np.isnan(aM2_mod))[0]],marker='D',label='mod',color='red')
    ...
    model_output_ax[0].xaxis.set_ticklabels([])
    model_output_ax[0].grid()
    model_output_ax[0].set_ylim(0,1)
    model_output_ax[0].set_ylabel('Amplitude M$_2$ tide [m]')
    
    model_output_ax[1].scatter(punten[np.where(~np.isnan(aM2_mod))[0]],pha_M2[np.where(~np.isnan(aM2_mod))[0]],marker='D',label='obs',color='blue')
    model_output_ax[1].scatter(punten[np.where(~np.isnan(aM2_mod))[0]],pM2_mod[np.where(~np.isnan(aM2_mod))[0]],marker='D',label='mod',color='red')
    ...
    model_output_ax[1].grid(),model_output_ax[1].set_ylim(0,180)
    model_output_ax[1].set_ylabel('Phase M$_2$ tide [deg]')
    model_output_ax[1].tick_params(labelrotation=90)
    
    model_output_ax[1].legend(bbox_to_anchor=(1., -.75))
    plt.show()
    '''
